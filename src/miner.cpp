...
// ***TODO*** ScanHash is not yet used in Collectiblecoin
//
...
{
    LogPrintf("CollectiblecoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("collectiblecoin-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("CollectiblecoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("CollectiblecoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("CollectiblecoinMiner:\n  proof-of-work found\n  hash: %s\n  target: %s\n", hash.GetHex(), hashTarget.GetHex());
    ...
    {
        LogPrintf("CollectiblecoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("CollectiblecoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}
