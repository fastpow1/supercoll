...
const int TrafficGraphData::CollectiblecoindQueueSizes[] = {
...
void TrafficGraphData::tryAddingSampleToStash(GraphRange range)
{
    ...
    if (queue.size() > CollectiblecoindQueueSizes[range]){
           sampleStash[range].push_front(queue.at(CollectiblecoindQueueSizes[range]));
    }
}
...
{
    SampleQueue& queue = sampleMap[range];
    if (queue.size() == CollectiblecoindQueueSizes[range] + 2){
