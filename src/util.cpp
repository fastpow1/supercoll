...
const char * const BITCOIN_CONF_FILENAME = "collectiblecoin.conf";
const char * const BITCOIN_PID_FILENAME = "collectiblecoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "collectiblecoin" is a composite category enabling all Collectiblecoin-related debug output
            if(ptrCategory->count(string("collectiblecoin"))) {
...
#else
    const char* pszModule = "collectiblecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CollectiblecoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CollectiblecoinCore
    // Mac: ~/Library/Application Support/CollectiblecoinCore
    // Unix: ~/.collectiblecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CollectiblecoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CollectiblecoinCore";
#else
    // Unix
    return pathRet / ".collectiblecoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty collectiblecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override collectiblecoin.conf
